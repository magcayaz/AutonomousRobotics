...
function getWabbleToBall()
	local dy = wcm.get_ball_y()+rightKickDist
	
	return dy
end

function getRotateWabble()
	local angle = 0
	local tx = -2.72  --goal coordinates
	local ty = 0
	local rx = wcm.get_pose().x
	local ry = wcm.get_pose().y
	local ra = wcm.get_pose().a*radToDeg
...
-- --------------------------------------------------------wabble to ball
wabbleToBallStart = function(hfa)   
	darwin.track()
	
	local w = getWabbleToBall()
	print("wabble amount" ..w)
	if w > rightKickDist+ rightKickThresh then
		print("wabbling left to ball");
		darwin.setVelocity(0, wabbleSpeed,0);
	elseif w < rightKickDist- rightKickThresh then
		darwin.setVelocity(0, -1*wabbleSpeed,0);
		print("wabbling right to ball");
	else
		print("stopping in wabble");
		darwin.stop();
	end
	
end	
wabbleToBallGo = function(hfa) 
end
wabbleToBallStop = function (hfa)
	darwin.stop()
	darwin.kickBall()
end
----_________________________________________________________Wabble around			

wabbleRotToBallStart = function(hfa) 
	darwin.track()
	local a = getRotateWabble()
	print("wabble rotation amount" ..a)
	if a > 0 then
		print("wabble rotate left to ball");
		darwin.setVelocity(0, walkSpeed*2,-2*turnSpeed);
	elseif a < 0 then
		darwin.setVelocity(0, -2*walkSpeed,2*turnSpeed);
		print("wabble rotate right to ball");
	else
		print("stopping in rotate wabble");
		darwin.stop();
	end
	
end	
wabbleRotToBallGo = function(hfa) 
end
wabbleRotToBallStop = function (hfa)
	darwin.stop()
end
...
ballCloseStart = function(hfa) 
	darwin.track()
	darwin.stop()
	
end	
ballCloseGo = function(hfa) 
end
ballCloseStop = function (hfa)
	darwin.stop()
end
...
-- MAKE STATES
ballCloseStart = makeBehavior("ballClose",ballCloseStart, ballCloseStop, ballCloseGo);
...
wabbleToBall = makeBehavior("wabbleToBall", wabbleToBallStart, wabbleToBallStop, wabbleToBallGo);
wabbleRotToBall = makeBehavior("wabbleRotToBall", wabbleRotToBallStart, wabbleRotToBallStop, wabbleRotToBallGo);
...
myMachine = makeHFA("myMachine", makeTransition(
	{
		...
		[ballClose] = function() 
			if vcm.get_ball_detect()==1 then
				if getTurnAngleToBall()[2] > ballLimit*5
					return ballClose
				else
					return turnToRelativeBall 
			else 
		...
		[walkForwardBall] = function() 
			...
			elseif getTurnAngleToBall()[2] > ballLimit then
				...
				return wabbleRotToBall   --relative localization of the ball.
			end 
		end,
		[wabbleRotToBall] = function() 
			if getRotateWabble()>angleLimit then 
				return wabbleRotToBall 
			else 
				return wabbleToBall 
			end 
		end,
		[wabbleToBall] = function() 
			if getWabbleToBall()<rightKickDist- rightKickThresh or getWabbleToBall() >rightKickDist+ rightKickThresh  then 
				return wabbleToBall 
			else 
				return stop 
			end 
